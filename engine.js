...
export const isSupported=code=>SUPPORTED.has(code)||SHOREKEEPER_SUPPORTED.has(code)||CAMELLYA_SUPPORTED.has(code);
...
export class Game{
 constructor(db,decks,{scenario=false,seedConcerto=true,rng=Math.random}={}){
  this.practice=scenario;this.seedConcerto=seedConcerto;this.lastDamage=null;this.lastDraw=null;this.lastTurnDraw=null;this.db=db;this.rng=rng;this.serial=0;this.turn=1;this.active=0;this.phase='setup';this.queue=[];this.choice=null;this.log=[];this.winner=null;this.revealed=[];this.after=null;this.follow=0;this.counterWinner=-1;
 ...
 beginTurn(){this.phase='start';this.players.forEach(p=>{p.used={};p.locked=false;p.noFollow=false;p.action.forEach(x=>x.bonus=0);});this.counterWinner=-1;this.follow=0;this.revealed=[];this.charEvent('start',this.active);this.after=()=>{this.phase='action';this.draw(this.active,this.turn===1?1:2,'turn');this.note('ช่วง Action • '+this.players[this.active].name);};if(!this.queue.length)this.next();}
 ...
 judge(){this.phase='judgement';this.counterWinner=judgement(...this.selected.map(x=>x?this.card(x):null),this.active);this.lastClash={cards:this.selected.map(x=>x?{code:x.code,uid:x.uid}:null),winner:this.counterWinner};this.follow=this.counterWinner>=0&&color(this.card(this.selected[this.counterWinner]))===red?Infinity:0;this.note(this.counterWinner<0?'ผล Counter: เสมอ':'ชนะ Counter: '+this.players[this.counterWinner].name);for(const i of [this.active,1-this.active])this.charEvent('judgement',i);for(const i of [this.active,1-this.active])if(this.selected[i])this.actionEvent('judgement',i,this.selected[i]);this.sortQueue();this.after=()=>{if(this.counterWinner>=0){this.damage(1-this.counterWinner,this.attack(this.counterWinner,this.selected[this.counterWinner]));this.check();if(this.winner===null)this.phase='combo';}else this.endCounter();};}
 attack(i,x){return Math.max(0,+this.card(x).stats.attack+(x.bonus||0));}
 combo(uid){const i=this.counterWinner,p=this.players[i],x=p.hand.find(x=>x.uid===uid);if(!x)throw Error('ไม่พบการ์ด');const reason=this.playReason(i,x,true);if(reason)throw Error(reason);this.pay(i,this.cost(i,x),()=>{p.action.push(this.remove(p,'hand',x));this.follow--;this.phase='comboSkill';this.actionEvent('combo',i,x);if(p.chars[p.leader].some(y=>y.code==='SD02-005')){x.bonus++;this.note('Jinshi Leader: Combo +1 ดาเมจ');}this.after=()=>{this.damage(1-i,this.attack(i,x));this.check();if(this.winner===null)this.phase='combo';};});}
