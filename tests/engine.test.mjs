...
test('Incarnation draws 3 and records 8 followups but current red-only demo policy blocks combo',()=>{const g=make();g.startCounter();g.select(find(g,0,'SD02-010'));g.select(find(g,1,'SD02-008'));assert.ok(g.choice);const pay=g.choice.items[1];g.answer([pay.uid]);assert.ok(g.players[0].trash.some(x=>x.uid===pay.uid));while(g.queue.length)g.activate(g.queue[0].id);assert.ok(g.players[0].locked);g.next();const q=g.queue.find(q=>q.x.code==='SD02-010');const before=g.players[0].hand.length;g.activate(q.id);assert.equal(g.players[0].hand.length,before+3);assert.equal(g.follow,8);g.next();assert.equal(g.comboAllowed,false);assert.throws(()=>g.combo(find(g,0,'SD01-017')),/ไม่มีสิทธิ์/);assert.equal(g.follow,8);});
