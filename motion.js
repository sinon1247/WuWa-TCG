let audioCtx=null;export function sfx(kind='move'){try{audioCtx??=new (window.AudioContext||window.webkitAudioContext)();const o=audioCtx.createOscillator(),g=audioCtx.createGain();const f={shuffle:180,draw:520,move:300,skill:720,clash:90,phase:240}[kind]||300;o.frequency.value=f;o.type=kind==='clash'?'sawtooth':kind==='skill'?'triangle':'sine';g.gain.setValueAtTime(.0001,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(kind==='clash'?.12:kind==='skill'?.1:.045,audioCtx.currentTime+.01);g.gain.exponentialRampToValueAtTime(.0001,audioCtx.currentTime+(kind==='clash'?.3:kind==='skill'?.45:.12));o.connect(g).connect(audioCtx.destination);o.start();o.stop(audioCtx.currentTime+.5);}catch{}}
...
export function hasCleanupMoves(snapshot,game){return snapshot?.game===game&&movements(snapshot.locations,locations(game)).some(m=>m.from==='action'&&m.to==='trash');}
export async function animateSkill(uid,label='SKILL ACTIVATED'){
 const card=cardElement(uid),reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;if(!card)return;
 sfx('skill');card.classList.add('skill-casting');const r=card.getBoundingClientRect(),layer=document.createElement('div');layer.className='skill-burst';layer.style.left=(r.left+r.width/2)+'px';layer.style.top=(r.top+r.height/2)+'px';const core=document.createElement('div');core.className='skill-core';const text=document.createElement('strong');text.textContent=label;layer.append(core,text);document.body.append(layer);
 if(reduced)await new Promise(resolve=>setTimeout(resolve,180));else await new Promise(resolve=>setTimeout(resolve,820));layer.remove();card.classList.remove('skill-casting');
}
...
export async function animatePhase(phase){sfx('phase');const labels={counterStart:'เริ่มช่วง COUNTER',setup:'เตรียมเกม',start:'เริ่มเทิร์น',draw:'จั่วการ์ด',action:'ACTION',select:'เลือก COUNTER',counter:'COUNTER SKILLS',judgement:'JUDGEMENT',combo:'FOLLOW-UP ATTACK',comboSkill:'COMBO SKILLS',counterEnd:'จบ COUNTER',end:'จบเทิร์น',finished:'จบเกม'};const layer=document.createElement('div');layer.className='phase-banner';layer.textContent='เข้าสู่ '+(labels[phase]||phase);document.body.append(layer);await new Promise(r=>setTimeout(r,matchMedia('(prefers-reduced-motion: reduce)').matches?220:950));layer.remove();}
